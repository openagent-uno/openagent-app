...
const runner = String(process.argv[2] || process.platform).toLowerCase();
...
let extensions;
let metadataName;
if (runner.includes('mac') || runner === 'darwin') {
  extensions = ['.dmg', '.zip'];
  metadataName = `${channel}-mac.yml`;
} else if (runner.includes('win')) {
  extensions = ['.exe'];
  metadataName = `${channel}.yml`;
} else if (runner.includes('linux')) {
  extensions = ['.AppImage', '.deb'];
  metadataName = `${channel}-linux.yml`;
} else {
  throw new Error(`Unsupported release runner: ${runner}`);
}
...
const expectedMetadataUrls = runner === 'macos' || runner === 'darwin'
  ? [
...
fs.writeFileSync(
  path.join(releaseDir, `release-manifest-${runner}.json`),
  `${JSON.stringify(manifest, null, 2)}\n`,
  { encoding: 'utf8', mode: 0o600 },
);

console.log(`packaged-artifact smoke: ${version} ${runner} ${channel} ok`);
